...
// A model list that legitimately shrinks between snapshots (a failed probe, a
// retired model) must not resurrect stale rows. Custom rows are derived from
// settings, so a missing custom row was removed by the user and is never
// resurrected.
