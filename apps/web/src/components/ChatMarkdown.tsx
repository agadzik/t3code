...
import {
  ARTIFACT_TEMPLATE_HAST_PROPERTIES,
  remarkFileCitationDirectives,
  renderFileCitationsAsMarkdown,
} from "@t3tools/client-runtime/file-citation-markdown-directives";
...
const CHAT_MARKDOWN_SANITIZE_SCHEMA = {
  ...
  attributes: {
    ...
    div: [...(defaultSchema.attributes?.div ?? []), ...ARTIFACT_TEMPLATE_HAST_PROPERTIES],
...
function useChatMarkdownState({
  ...
  const markdownFileLinkMetaByHref = useMemo(() => {
    ...
    >();
    for (const href of extractMarkdownLinkHrefs(renderFileCitationsAsMarkdown(text))) {
