    ...
    async (candidate: ProviderSettingsUpdateCandidate) => {
      ...
      if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
        ...
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: `Could not update ${getDriverOption(candidate.driver)?.label ?? candidate.driver}`,
            description:
  ...
  const renderProviderInstance = (row: InstanceRow, mode: "list" | "editor") => {
    ...
    return (
      <ProviderInstanceCard
        ...
        setup={
          mode === "editor" &&
          (liveProvider?.setup?.canAuthenticate === true ||
            liveProvider?.setup?.canInstall === true) ? (
