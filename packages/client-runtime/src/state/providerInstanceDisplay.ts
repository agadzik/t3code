 ...
 * `work_account` becomes "Work Account" and `myCustomInstance` becomes
 * "My Custom Instance".
 */
 ...
 *
 *   1. A snapshot `displayName` that differs from the driver's slug label —
 ...
 *      kind), so the slug is what keeps two instances of one driver apart.
 *   3. The snapshot's `displayName`, or the prettified driver slug.
 */
export function resolveProviderInstanceDisplayName(
  ...
  const kindLabel = humanizeSlug(snapshot.driver);
