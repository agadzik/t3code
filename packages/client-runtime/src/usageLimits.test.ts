...
describe("providersWithLimits", () => {
  it("keeps only usable providers whose driver reports limits at all", () => {
    ...
    const testDriver = provider({ usageLimits: limits });
    expect(
      providersWithLimits([
        testDriver,
        provider({
...
describe("pools", () => {
  ...
  const otherDriver = ProviderDriverKind.make("otherDriver");
  const source = {
  ...
  it("merges one account reported natively on two environments and by a hub into one entry", () => {
            ...
            usageLimitSources: [
              {
                ...
                accounts: [
                  {
                    id: "otherDriver-same@example.com.json",
                    driver: otherDriver,
    ...
    expect(accounts[0]).toMatchObject({
      key: "env-a:otherDriver",
      sourceLabel: null,
      // Desktop's read is fresher, so its credits and its redeem are the ones on show.
      redeem: { environmentId: "env-b", input: { instanceId: "otherDriver" } },
      environments: [
  ...
  it("redeems through the hub when it holds a credit, even with a fresher native read", () => {
    ...
    expect(account?.redeem).toEqual({
      ...
      input: {
        sourceId: "hub",
        accountId: "otherDriver-same@example.com.json",
        creditId: "hub-credit",
      },
    });
  ...
  it("uses the freshest hub credit and its environment even when the account is also native", () => {
    ...
    expect(account?.redeem).toEqual({
      environmentId: "env-b",
      input: { sourceId: "hub", accountId: "testDriver-same.json", creditId: "credit-2" },
    });
  ...
  it("keys a hub account without an email by hub, so two environments on one hub share it", () => {
    ...
    const accounts = collectLimitAccounts(input);
    expect(accounts.map((account) => account.key)).toEqual(["hub:otherDriver-team-seat.json"]);
    expect(accounts[0]?.displayName).toBe("otherDriver-team-seat");
  });
...
describe("/usage-limits", () => {
  ...
  it("supports a source-only provider and keeps duplicates when the native probe failed", () => {
    ...
    ).toEqual(["testDriver", "hub:duplicate", "hub:oss"]);
  ...
  it("surfaces source errors only for sources that carry the selected driver", () => {
    ...
    const otherDriverOnly = { ...failing, accounts: failing.accounts.slice(2) };
    expect(
      collectProviderUsageLimits(selected.instanceId, [selected], [otherDriverOnly], now)?.notices,
    ).toEqual([]);
...
describe("sameUsageLimitCommandCoverage", () => {
  ...
  it("ignores quota movement but not the drivers offered the command", () => {
    const withTestDriver = [{ ...base, accounts: [testDriverAccount] }];
    const withTestDriverLater = [
      {
        ...base,
        accounts: [
          {
    ...
    expect(sameUsageLimitCommandCoverage(withTestDriver, withTestDriverLater)).toBe(true);
    expect(sameUsageLimitCommandCoverage(withTestDriver, [{ ...base, accounts: [] }])).toBe(false);
  });
